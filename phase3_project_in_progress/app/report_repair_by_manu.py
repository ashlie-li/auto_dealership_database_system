...
def repair_by_manu():
    ...
    with open(os.path.join(os.getcwd(), 'sql_files', 'report_repair_by_manu.sql'), "r", encoding='utf-8') as file:
        tmp = file.readlines()
    query = " ".join(tmp)
    print(query)
    ...
    return render_template('report_repair_by_manu.html', col = col, res = res_list, role=role)
...
def repair_manu_detail(manu):
    ...
    return render_template('report_repair_by_manu_detail.html', col = col, res = res, role=role)