...
def add_sales(Vin):
    ...
    if request.method == 'POST' and 'CustomerID' in request.form:
        ...
        if 'SalePrice' not in request.form:
            return render_template('sales.html', CustomerID=CustomerID, Vin=Vin, msg=msg, role=role)
            ...
            if int(res0) == 0 and session['role'] != 'Owner':
                return render_template('sales.html', CustomerID=CustomerID, Vin=Vin, msg='Price too Low!', role=role)
