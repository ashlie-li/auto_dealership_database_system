With SingleColorSoldVehicles AS 
                ...
                WHERE  DATEDIFF((SELECT MAX(SaleDate) FROM SalesEvents) , SaleDate)  <= 30)  AS MonthlySales,
                (SELECT COUNT(*) FROM MultiColorSoldVehicles 
                WHERE DATEDIFF((SELECT MAX(SaleDate) FROM SalesEvents) , SaleDate)  <= 365)  AS YearSales,
